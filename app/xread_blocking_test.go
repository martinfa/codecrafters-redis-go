...
func TestParseBlockingXreadCommandWithDollarStartID(t *testing.T) {
	streamKeys, startIDs, blockMilliseconds, errorResponse := parseXreadCommand(&RedisCommand{
		Type: CmdXREAD,
		Args: []string{"BLOCK", "1000", "streams", "stream_key", "$"},
	})
	...
	if len(startIDs) != 1 || startIDs[0] != "$" {
		t.Fatalf("startIDs = %v, expected [$]", startIDs)
	}
}

func TestHandleBlockingXreadWithDollarStartIDWaitsForNewEntry(t *testing.T) {
	resetXreadTestState(t)
	...
	go func() {
		resultChannel <- HandleXread(&RedisCommand{
			Type: CmdXREAD,
			Args: []string{"BLOCK", "0", "streams", "stream_key", "$"},
		})
	}()
	...
	case <-time.After(500 * time.Millisecond):
		t.Fatal("timed out waiting for blocking XREAD with $ response")
	}
}

func TestHandleBlockingXreadWithDollarStartIDReturnsNullArrayOnTimeout(t *testing.T) {
	resetXreadTestState(t)
	...
	result := HandleXread(&RedisCommand{
		Type: CmdXREAD,
		Args: []string{"BLOCK", "100", "streams", "stream_key", "$"},
	})
